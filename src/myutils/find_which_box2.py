...
class TableAffineClass(BaseAffineClass):
    ...
    def visualize_table_and_boxes_v2(self, boxes, save=False, file_name=None, save_dir=None,tilt=False):
        """
        可视化表格和框的位置，并显示IOU值
        """
        ...
        # 绘制表格网格
        self._draw_table_grid(ax, top_left, top_right)
        # [1941.6661376953125, 1019.8320922851562, 2139.2294921875, 1201.1162109375]
        # 处理每个框
        for box in boxes:
            ...
            # 获取框所在的格子和IOU值
            cell_positions, area_ratios = self.get_cell_position_v3(  #横这种方式下，cell_position_v3返回的是空的。
                x1, y1, x2, y2, self.top_pos, self.new_row_height,
                self.col_width, self.rows, self.cols
            )

            # 高亮框所在的格子并显示IOU值
            for (row, col), iou in zip(cell_positions, area_ratios):
                cell_x = top_left[0] + col * self.col_width
                cell_y = top_left[1] + row * self.new_row_height

                # 添加半透明背景
                ax.add_patch(patches.Rectangle(
                    (cell_x, cell_y), self.col_width, self.new_row_height,
                    linewidth=1, edgecolor='blue', facecolor='lightblue', alpha=0.3
                ))

                # 显示IOU值
                # ax.text(
                #     cell_x + self.col_width / 2, cell_y + self.new_row_height / 2,
                #     f'a_r: {iou:.2f}', ha='center', va='center',
                #     color='red', fontweight='bold'
                # )

        # 绘制标记点和距离
        self._draw_markers_and_distances(ax)

        # 设置图表属性
        self._set_plot_properties(ax, top_left)

        # 保存或显示图像
        if save and save_dir and file_name:
            save_path = save_dir / f'{file_name}sheet_topology.png'
            plt.savefig(save_path)
        else:
            plt.show()

    def visualize_table_and_boxes_v3(self, boxes,quad_xyes, save=False, file_name=None, save_dir=None,tilt=False):
        """
        可视化表格和框的位置，并显示IOU值
        """
        fig, ax = plt.subplots(figsize=(10, 8))
        ...
        # 处理每个框
        if tilt:
            for quad_xy in quad_xyes:
                # 生成旋转矩形
                quad_xy=np.array(quad_xy,dtype=np.float32)
                rectangle, angle_rad = process_quad(quad_xy)  # angle_rad弧度

                # 绘制旋转矩形
                rotated_patch = patches.Polygon(
                    rectangle, closed=True, linewidth=1, edgecolor='green', facecolor='none'
                )
                ...
                ax.text(
                    center[0], center[1], f'{np.degrees(angle_rad):.2f}°',
...
class MarkerAffineClass(BaseAffineClass):
    ...
    def visualize_markers_and_boxes(self, boxes, quad_xyes=None, save=False, file_name=None, save_dir=None, tilt=False):
        """可视化锚点和框的位置"""
        fig, ax = plt.subplots(figsize=(10, 8))
        ...
        if tilt and quad_xyes is not None:
            for quad_xy in quad_xyes:
                quad_xy = np.array(quad_xy, dtype=np.float32)
                rectangle, angle_rad = process_quad(quad_xy)
                
                # 绘制旋转矩形
                rotated_patch = patches.Polygon(
                    rectangle, closed=True, linewidth=2, edgecolor='green', facecolor='none'
                )
                ax.add_patch(rotated_patch)
        ...
        max_x = max(self.mark1_pos[0], self.mark2_pos[0], self.mark3_pos[0], self.mark4_pos[0]) * 1.2
        max_y = max(self.mark1_pos[1], self.mark2_pos[1], self.mark3_pos[1], self.mark4_pos[1]) * 1.2
        ax.set_xlim([0, max_x])
        ax.set_ylim([0, max_y])
        ax.invert_yaxis()
        ax.set_title('Markers and Box Visualization')
        plt.grid(False)
        plt.legend()

        # 保存或显示图像
        if save and save_dir and file_name:
            save_path = save_dir / f'{file_name}_markers.png'
            plt.savefig(save_path)
        else:
            plt.show()
